...
const defaultSettings = {
    ...
    tableStructure: [
        {
            tableName: "时间管理", tableIndex: 0, columns: ['日期', '时间', '地点（当前描写）', '此地角色'], columnsIndex: [0, 1, 2, 3], enable: true, Required: true, asStatus: true, toChat: true, note: "记录剧情时间地点相关的表格，应保持在一行",
            initNode: '本轮需要记录当前剧情时间、地点、人物信息，使用insertRow函数，确保上下文衔接自然，没有大幅时间异常跳转', updateNode: "当描写的场景，时间，人物变更时", deleteNode: "此表大于一行时应删除多余行"
        },
        {
            tableName: '角色特征', tableIndex: 1, columns: ['角色名', '身体特征', '性格', '职业', '爱好', '喜欢的事物（作品、虚拟人物、物品等）', '住所', '角色关系', '互动方式', '其他重要信息'], enable: true, Required: true, asStatus: true, toChat: true, columnsIndex: [0, 1, 2, 3, 4, 5, 6, 7], note: '角色天生或不易改变的特征csv表格，随剧情改变的角色关系变化，事无巨细罗列出，并思考本轮有否有其中的角色，根据角色设定应作出的反应',
        ...
        {
            tableName: '角色与<user>社交表格', tableIndex: 2, columns: ['角色名', '对<user>关系', '对<user>态度', '对<user>好感'], columnsIndex: [0, 1, 2, 3], enable: true, Required: true, asStatus: true, toChat: true, note: '思考如果有角色和<user>互动，应什么态度，该数值影响角色对<user>的态度，细化情感变化，过渡自然',
        ...
        {
            tableName: '任务、命令或者约定表格', tableIndex: 3, columns: ['角色', '任务', '地点', '持续时间', '人物进度'], columnsIndex: [0, 1, 2, 3], enable: true, Required: false, asStatus: true, toChat: true, note: '思考本轮是否应该执行任务/赴约，任务进度变化和完成度',
            insertNode: '当特定时间约定一起去做某事时/某角色收到做某事的命令或任务时', updateNode: "", deleteNode: "当大家赴约时/任务或命令完成时/任务，命令或约定被取消时"
        },
        {
            tableName: '重要事件历史表格', tableIndex: 4, columns: ['角色', '事件简述', '日期', '地点', '情绪', '持续时间'], columnsIndex: [0, 1, 2, 3, 4], enable: true, Required: true, asStatus: true, toChat: true, note: '记录<user>或角色经历的重要事件',
            initNode: '本轮必须从上文寻找可以插入的事件并使用insertRow插入', insertNode: '当某个角色经历让自己印象深刻的事件时，比如表白、分手等', updateNode: "", deleteNode: ""
        },
        {
            tableName: '重要物品表格', tableIndex: 5, columns: ['拥有人', '物品描述', '物品名', '重要原因', '态度'], columnsIndex: [0, 1, 2, 3], enable: true, Required: false, asStatus: true, toChat: true, note: '对某人很贵重或有特殊纪念意义的物品',
            insertNode: '当某人获得了贵重或有特殊意义的物品时/当某个已有物品有了特殊意义时', updateNode: "", deleteNode: ""
        },
    ],
    to_chat_container: `<div class="table-preview-bar"><details> <summary>记忆表格</summary>
$0
</details></div>
...
-输出表格内容时需保持详尽精准的内容，分析上下文，不错过任何细节，每个表格至少20字
...
`,
    refresh_system_message_template: `你是名为aw，擅长专业表格整理，对与上下文内容你了解的极其透彻，不会错过任何细节以及情感变化，你的用词细腻详尽，请严格按照<user>的指令和格式要求处理表格数据。`,
...
<整理规则>
    ...
    6. 在表格的内容需要尽可能精确详尽，与上下文聊天记录的内容完全对应，输出内容每段至少20字
